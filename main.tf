...
resource "aws_subnet" "subnet_publica_1" {
  # checkov:skip=CKV_AWS_130: Requerido en AWS Academy para asignacion de IP Publica al Web Server
  vpc_id                  = aws_vpc.mi_vpc.id
...
# --- SECURITY GROUP ---
# checkov:skip=CKV_AWS_260: Requerido por la actividad para exponer el servidor web HTTP publico
# checkov:skip=CKV2_AWS_5: El SG se asocia a la instancia en un modulo remoto externo de computo
resource "aws_security_group" "sg_computo" {
  ...
  ingress {
    description = "Acceso SSH seguro restringido"
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
  ...
  ingress {
    description = "Acceso Web HTTP publico"
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
...
resource "aws_cloudwatch_log_group" "vpc_log_group" {
  # checkov:skip=CKV_AWS_158: No es viable usar llaves KMS administradas en AWS Academy
